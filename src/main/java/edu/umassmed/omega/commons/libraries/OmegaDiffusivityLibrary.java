...
import main.java.edu.umassmed.omega.commons.errorInterpolation.SplineInterpolation;
import main.java.edu.umassmed.omega.commons.errorInterpolation.SplineInterpolation.Size;
...
public class OmegaDiffusivityLibrary {
	...
	public static Double computeMinimumDetectableD(
			final Map<OmegaFrame, Double> frameMinSNR) {
		// find the minimum SNR for the current dataset
		Double minMinSNR = Double.MAX_VALUE;
		for (final OmegaFrame f : frameMinSNR.keySet()) {
			final Double snr = frameMinSNR.get(f);
			if (minMinSNR > snr) {
				minMinSNR = snr;
			}
		}
		...
		try {
			...
			OmegaLogFileManager.handleCoreException(e);
			return null;
		}
	}
